# -*- encoding: utf-8 -*-
"""
@Time    :   2025-08-01 16:35:04
@desc    :   ref: https://zhuanlan.zhihu.com/p/38753080790
@Author  :   ticoAg
...
class MarkdownHeaderTextSplitter:
    ...
    def __init__(
        ...
        chunk_overlap: int = 0,  # 添加 chunk_overlap 参数
    ):
        ...
        self._chunk_overlap = chunk_overlap  # 保存重叠参数
        # 预编译正则表达式（如果需要）
            ...
            if split_needed:
                ...
                if (
                    ...
                    if self._chunk_overlap > 0:
                        # 从上一个块的末尾获取重叠行
                        overlap_length = 0
                        for l in reversed(lines_to_chunk):
                            line_length = self._length_function(l) + 1
                            if overlap_length + line_length > self._chunk_overlap:
                                break
                            overlap_lines.insert(0, l)
                            overlap_length += line_length

                    # 开始新的子块，包含重叠行、剩余行和当前行
                    current_lines = overlap_lines + remaining_lines + [line]
